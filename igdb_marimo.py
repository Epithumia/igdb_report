...
# Redefines some code so that Marimo will show cell data and hopefully progress bars


class IGDB:
    def __init__(self, csv_dir: str, mo) -> None:
        """
    ...
    def download_igdb_csv(self, endpoint: str, destination: str) -> None:
        """
        Parameters
        ----------
        endpoint : str
    ...
    def download_igdb_csv_dumps(self):
        ...
        with self.mo.status.progress_bar(
            total=len(endpoint_list), title="Getting endpoints..."
        ) as bar:
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            with self.mo.status.progress_bar(total=len(endpoints), title="Downloading files...") as bar:
                futures = []
                for endpoint in endpoints:
                    filename = endpoint + ".csv"
                    destination = os.path.join(self.csv_dir, filename)
    ...
    def load_database(self):
        ...
        with self.mo.status.progress_bar(
            total=len(self.endpoints), title="Préparation..."
        ) as bar:
        ...
        with self.mo.status.progress_bar(
            total=len(update_queries), title="Transformation des listes..."
        ) as bar:
    ...
    def check_empty_endpoints(self):
        ...
        if no_empty_endpoints:
            ...
            return ITable(df, buttons=["copyHtml5", "csvHtml5"])
    ...
    def query(self, query):
        ...
        try:
            return ITable(
                self.cnx.sql(query).to_df(), buttons=["copyHtml5", "csvHtml5"]
            )
